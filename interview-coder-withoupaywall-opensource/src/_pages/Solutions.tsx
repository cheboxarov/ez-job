    ...
    <div className="space-y-2 relative">
      ...
      {isLoading ? (
        ...
        <div className="w-full relative">
          ...
          >
            {copied ? t("solutions.copied") : t("solutions.copy")}
          </button>
          <SyntaxHighlighter
            showLineNumbers
...
export const ComplexitySection = ({
  ...
  const formatComplexity = (complexity: string | null): string => {
    ...
    if (!complexity || complexity.trim() === "") {
      return t("solutions.notAvailable");
    }
  ...
  return (
    <div className="space-y-2">
      <h2 className="text-[13px] font-medium text-white tracking-wide">
        {t("solutions.complexity")}
      </h2>
      {isLoading ? (
        <p className="text-xs bg-gradient-to-r from-gray-300 via-gray-100 to-gray-300 bg-clip-text text-transparent animate-pulse">
          {t("solutions.calculating")}
        </p>
      ) : (
        <div className="space-y-3">
          <div className="text-[13px] leading-[1.4] text-gray-100 bg-white/5 rounded-md p-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <strong>{t("solutions.time")}</strong> {formattedTimeComplexity}
              </div>
            </div>
          </div>
          <div className="text-[13px] leading-[1.4] text-gray-100 bg-white/5 rounded-md p-3">
            <div className="flex items-start gap-2">
              <div className="w-1 h-1 rounded-full bg-blue-400/80 mt-2 shrink-0" />
              <div>
                <strong>{t("solutions.space")}</strong> {formattedSpaceComplexity}
              </div>
            </div>
          </div>
        </div>
...
const Solutions: React.FC<SolutionsProps> = ({
  ...
  useEffect(() => {
    ...
    const cleanupFunctions = [
      ...
      window.electronAPI.onProcessingNoScreenshots(() => {
        showToast(
          t("errors.noScreenshotsTitle"),
          t("errors.noExtraScreenshots"),
          "neutral"
        )
      }),
  ...
  }, [isTooltipVisible, tooltipHeight, t])
            ...
            <div className="rounded-lg overflow-hidden">
              <div className="px-4 py-3 space-y-4 max-w-full">
                {!solutionData && (
                  <>
                    <ContentSection
                      title={t("solutions.problemStatement")}
                ...
                {solutionData && (
                  <>
                    <ContentSection
                      title={t("solutions.myThoughtsWithShortcut", {
                        shortcut: `${COMMAND_KEY} + ${t("shortcuts.scrollHint")}`
                      })}
                      content={
                        thoughtsData && (
                          <div className="space-y-3">
