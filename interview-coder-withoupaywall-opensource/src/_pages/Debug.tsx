  ...
  useEffect(() => {
    ...
    if (newSolution) {
      ...
      if (newSolution.debug_analysis) {
        ...
        setNewCode(newSolution.code || t("debug.debugMode"));
        ...
        if (newSolution.debug_analysis.includes('\n\n')) {
          ...
          setThoughtsData([t("debug.analysisFallback")]);
        }
      } else {
        // Fallback to code or default
        setNewCode(newSolution.code || t("debug.noAnalysis"));
        setThoughtsData(newSolution.thoughts || [t("debug.analysisFallback")]);
      }
      setTimeComplexityData(newSolution.time_complexity || t("debug.debugModeLabel"))
      setSpaceComplexityData(newSolution.space_complexity || t("debug.debugModeLabel"))
    ...
    const cleanupFunctions = [
      ...
      window.electronAPI.onDebugSuccess((data) => {
        ...
        if (data.debug_analysis) {
          ...
          // Set code separately for the code section
          setNewCode(data.code || t("debug.debugMode"));
          ...
          // Fallback to code or default
          setNewCode(data.code || t("debug.noAnalysis"));
          setThoughtsData(data.thoughts || [t("debug.analysisFallback")]);
          setDebugAnalysis(null);
        }
        setTimeComplexityData(data.time_complexity || t("debug.debugModeLabel"));
        setSpaceComplexityData(data.space_complexity || t("debug.debugModeLabel"));
