...
function App() {
  ...
  const initializationRef = useRef(false) // Track if initialization has been attempted
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      try {
        ...
        const getConfigPromise = window.electronAPI?.getConfig();
        ...
        if (!config) {
          console.warn("Config load timed out, continuing with defaults.")
        } else if (config.interfaceLanguage) {
        ...
        // and stored in config as a single model field
    ...
    return () => {
      ...
      // Don't reset initialization state on cleanup to prevent re-initialization loops
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []) // Empty dependency array - run only once on mount, functions are stable via useCallback
