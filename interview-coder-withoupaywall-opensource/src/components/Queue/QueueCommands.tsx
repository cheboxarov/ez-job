...
const QueueCommands: React.FC<QueueCommandsProps> = ({
  ...
  const handleSignOut = async () => {
    try {
      ...
      showToast(t('common.successTitle'), t('success.loggedOut'), 'success');
      ...
      showToast(t('common.errorTitle'), t('errors.failedToLogOut'), 'error');
    }
  }
  ...
  const screenshotLabel = () => {
    if (screenshotCount === 0) return t("queue.takeFirstScreenshot")
    if (screenshotCount === 1) return t("queue.takeSecondScreenshot")
    if (screenshotCount === 2) return t("queue.takeThirdScreenshot")
    if (screenshotCount === 3) return t("queue.takeFourthScreenshot")
    if (screenshotCount === 4) return t("queue.takeFifthScreenshot")
    return t("queue.nextReplacesFirst")
  }
            ...
            onClick={async () => {
              try {
                ...
                if (!result.success) {
                  ...
                  showToast(
                    t("common.errorTitle"),
                    t("errors.failedToTakeScreenshot"),
                    "error"
                  )
                }
              } catch (error) {
              ...
              >
                ...
                <div className="p-3 text-xs bg-black/80 backdrop-blur-md rounded-lg border border-white/10 text-white/90 shadow-lg">
                  <div className="space-y-4">
                    <h3 className="font-medium truncate">{t("queue.keyboardShortcuts")}</h3>
                    <div className="space-y-3">
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="truncate">{t("queue.toggleWindow")}</span>
                        ...
                        <p className="text-[10px] leading-relaxed text-white/70 truncate mt-1">
                          {t("queue.toggleWindowDescription")}
                        </p>
                      </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="truncate">{t("queue.takeScreenshot")}</span>
                          <div className="flex gap-1 flex-shrink-0">
                            <span className="bg-white/20 px-1.5 py-0.5 rounded text-[10px] leading-none">
                              {COMMAND_KEY}
                            </span>
                        ...
                        <p className="text-[10px] leading-relaxed text-white/70 truncate mt-1">
                          {t("queue.takeScreenshotDescription")}
                        </p>
                      </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="truncate">{t("queue.solve")}</span>
                          <div className="flex gap-1 flex-shrink-0">
                            <span className="bg-white/20 px-1.5 py-0.5 rounded text-[10px] leading-none">
                              {COMMAND_KEY}
                            </span>
                        ...
                        <p className="text-[10px] leading-relaxed text-white/70 truncate mt-1">
                          {screenshotCount > 0
                            ? t("queue.solveDescription")
                            : t("queue.solveDescriptionEmpty")}
                        </p>
                      </div>
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            if (!result.success) {
                              ...
                              showToast(
                                t("common.errorTitle"),
                                result.error || t("errors.failedToDeleteScreenshot"),
                                "error"
                              )
                            }
                          } catch (error) {
                            ...
                            showToast(
                              t("common.errorTitle"),
                              t("errors.failedToDeleteScreenshot"),
                              "error"
                            )
                          }
                        }}
                      >
                        <div className="flex items-center justify-between">
                          <span className="truncate">{t("queue.deleteLastScreenshot")}</span>
                          <div className="flex gap-1 flex-shrink-0">
                            <span className="bg-white/20 px-1.5 py-0.5 rounded text-[10px] leading-none">
                              {COMMAND_KEY}
                            </span>
                        ...
                        <p className="text-[10px] leading-relaxed text-white/70 truncate mt-1">
                          {screenshotCount > 0
                            ? t("queue.deleteScreenshotDescription")
                            : t("queue.noScreenshotsToDelete")}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="pt-3 mt-3 border-t border-white/10">
                      ...
                      <div className="mb-3 px-2">
                        ...
                        >
                          <span className="text-[11px] text-white/70">{t("queue.language")}</span>
