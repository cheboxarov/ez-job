...
1.  **НЕ ЗАДАВАЙ ВОПРОСЫ**. Поле `questions` должно быть пустым.
2.  **НЕ ИСПОЛЬЗУЙ MARKDOWN**. В `new_text` только простой текст.
3.  **НЕ МЕНЯЙ ВЕСЬ ТЕКСТ**. Только нужные фрагменты.
4.  **НЕ ИСПОЛЬЗУЙ ЯКОРЯ**. Поля `anchor_before` и `anchor_after` больше не поддерживаются. Используй только `start_line` и `end_line`.
5.  **ОБЯЗАТЕЛЬНО УКАЗЫВАЙ НОМЕРА СТРОК**. Поля `start_line` и `end_line` обязательны для всех типов патчей.
6.  **ТОЧНОСТЬ `old_text`**. Для `replace` и `delete` поле `old_text` должно содержать точную копию текста из указанных строк резюме, включая все пробелы и переносы строк.
