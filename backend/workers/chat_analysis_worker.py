...
async def process_chats_cycle(config: AppConfig) -> None:
    ...
    async with UnitOfWork(session_factory) as uow:
        ...
        user = await uow.user_repository.get_first()
        if user is None:
        ...
        if auth_data is None:
            logger.warning(f"Auth данные для пользователя {user.id} не найдены, пропускаем цикл")
            return
        ...
        try:
            ...
            logger.error(f"Ошибка при фильтрации чатов: {exc}", exc_info=True)
            return
        ...
        try:
            ...
            logger.error(f"Ошибка при получении детальной информации о чатах: {exc}", exc_info=True)
            return
        ...
        try:
            ...
            logger.error(f"Ошибка при анализе чатов: {exc}", exc_info=True)
